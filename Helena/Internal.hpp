...
namespace Helena::Internal
{
    ...
    inline constexpr auto NameOf = entt::type_name<Type>().value();
    ...
    template <typename T>
    using remove_cvrefptr_t = std::conditional_t<std::is_array_v<T>, std::remove_all_extents_t<T>,
        std::conditional_t<std::is_pointer_v<T>, remove_cvref_t<std::remove_pointer_t<T>>, remove_cvref_t<T>>>;
    ...
    struct is_pair<T, std::void_t<
        decltype(std::declval<typename T::first_type>()),
        decltype(std::declval<typename T::second_type>())>> : std::true_type {};
    ...
    struct is_mapping<Container, std::enable_if_t<is_pair_v<
        typename std::iterator_traits<
        typename Container::iterator>::value_type>>> : std::true_type {};
    ...
    inline constexpr std::decay_t<decltype(Value)> is_always_value = Value;

//#ifdef HF_STANDARD_CPP17
//    template<class InputIt, class ForwardIt>
//    [[nodiscard]] inline constexpr InputIt find_first_of(InputIt first, InputIt last, ForwardIt s_first, ForwardIt s_last) noexcept {
...
//#endif
    ...
    [[nodiscard]] inline auto AddOrGetTypeIndex(Container& container, const Key typeIndex) -> decltype(auto) {
